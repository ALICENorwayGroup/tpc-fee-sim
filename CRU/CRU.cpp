...
{		
		if(!output_fifo.empty())
		{
			...
			//The real time it takes to send the packet through output link
			//Packet size / Throughput * 10^9 ns
			//(x number of samples * 10 bit + 50 bit header) / 10 * 10^9 b/s * 10^9 ns
			wait(((0.0 + temp.numberOfSamples * 10 + 50) / 128) * 1000000, SC_FS);	
			
			//The worst case, first packet must be sent in 100% after that it can be deleted from the buffer
			output_fifo.pop();

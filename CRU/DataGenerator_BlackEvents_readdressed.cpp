 ...
 * _remapped_overTime
 * DataGenerator_black_events_remapped_dynamiclly
 * 
 * */
	...
	while(currentTimeWindow < constants::NUMBER_TIME_WINDOWS_TO_SIMULATE)
	{
		...
		{	
			...
			//if(signalArray[currentSample-1][i].signalStrength != 0)														//i%495
			{
				...
				//Sample sample(currentTimeWindow, packetCounter, signalArray[currentSample-1][i].signalStrength);	//i%495	Sample sample(currentTimeWindow, packetCounter, signalArray[currentSample-1][i%495].signalStrength);
